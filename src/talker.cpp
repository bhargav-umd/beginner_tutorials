...
#include <sstream>
     ...
     *                * the easiest way to do it.  The third argument to init()
     * is the name of the node.
     *                   *
     *                      * You must call one of the versions of ros::init()
     ...
     * master
     *                * node will notify anyone who is trying to subscribe to
     * this topic name,
     *                   * and they will in turn negotiate a peer-to-peer
     * connection with this
     *                      * node.  advertise() returns a Publisher object
     * which allows you to
     *                         * publish messages on that topic through a call
     * to publish().  Once
     *                            * all copies of the returned Publisher object
     * are destroyed, the topic
     ...
     *                                           * than we can send them, the
     * number here specifies how many messages to
     *                                              * buffer up before throwing
     * some away.
     *                                                 */
    ...
    while (ros::ok()) {
         ...
         * with the type
         *                * given as a template parameter to the advertise<>()
         * call, as was done
